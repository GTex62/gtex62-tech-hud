...
- Optional: set `layout.positions.<widget>.anchor = "clock_center"` to treat `x/y` as offsets from the clock center. Add `anchor_scale = false` to keep those offsets fixed when layout scale changes. Use `anchor_point` (`top_left`, `top_right`, `bottom_left`, `bottom_right`, `center`) to pin a specific widget corner to the clock center.
- Embedded corners: set `theme.lua` → `embedded_corners.enabled = true` and set `embedded_corners.*.anchor` (`top_left`, `top_right`, `bottom_left`, `bottom_right`). Offsets are from the clock center using the time widget size; optional `embedded_corners.margin` and per-widget `x/y` are nudges. When enabled, `scripts/start-conky.sh` skips launching the standalone SYS/NET/STATION/BARO widgets.
...
- **Usage notes**:
  ...
  - When `embedded_corners.enabled = true`, corner widgets are drawn relative to the clock center inside the time widget.
...
- **Usage notes**:
  ...
  - The forecast strip and `weather.vline.*` follow `theme.scale` (when `layout.tie_draw_scale=true`).
...
- **Usage notes**:
  ...
  - `system.circle_outer.radius` / `radius_offset` are base values that scale with `theme.scale`.
...
- **Usage notes**:
  ...
  - Notes panel geometry and text offsets follow `theme.scale` when `layout.tie_draw_scale=true`.
...
- **Usage notes**:
  ...
  - Doctor font size follows `theme.scale` when `layout.tie_draw_scale=true`.
