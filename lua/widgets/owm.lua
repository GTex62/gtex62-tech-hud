--[[
  ${CONKY_SUITE_DIR}/lua/owm.lua
  Unified OpenWeather + forecast + aviation (optional horizon/planets)

  Overview
  --------
  This module is the "weather brain" for the Conky suite. It handles:
    • Loading layout + style from ${CONKY_SUITE_DIR}/theme.lua (THEME.weather.*)
    ...
    • Optional horizon arc + sun/moon/planets (available but not used by all widgets)
  ...
  ------------------------
    ...
    • OpenWeather caches (JSON, created by your fetch script):
        ${CONKY_CACHE_DIR}/owm_current.json
        ${CONKY_CACHE_DIR}/owm_daily.json      -- or OWM_DAILY_CACHE override
        ${CONKY_CACHE_DIR}/owm_forecast.json   -- forecast tiles (owm_fc_reduce.sh)
    • Sky variables (planet positions, etc; optional):
        ${CONKY_CACHE_DIR}/sky.vars
    • Helper scripts:
        ${CONKY_SUITE_DIR}/scripts/moon_times.sh
        ${CONKY_SUITE_DIR}/scripts/owm_fc_5rows.sh
        ${CONKY_SUITE_DIR}/scripts/metar_ob.sh
        ${CONKY_SUITE_DIR}/scripts/taf_wrap.sh
        ${CONKY_SUITE_DIR}/scripts/airsig_filter.sh

  Conky calls (lua_parse)
  -----------------------
  Used in tech-hud (via lua/widgets/weather.lua):
  ...
  Optional / available (currently disabled in gtex62-tech-hud):
    ...
    ${lua_parse owm sun_labels}                → Sunrise/sunset labels at arc ends
...
if type(THEME) ~= "table" or next(THEME) == nil then
  if type(theme) == "table" then THEME = theme end -- some themes set global
end
...
local function clock_center()
  if conky_window then
    return (conky_window.width / 2), (conky_window.height / 2)
  end
  return 0, 0
end

--[[ gtex62-tech-hud: unused arc center helper
local function weather_center()
  local wx, wy = weather_widget_offsets()
  local cx0, cy0 = clock_center()
  return cx0 + wx, cy0 + wy
end
]]
...
---------------------------------------------------
-- Optional overrides from ${CONKY_SUITE_DIR}/config/owm.vars
-- Only used for cache paths; all layout comes from theme.lua.

local VARS_FILE = SUITE_DIR .. "/config/owm.vars"
local VARS = (util.parse_vars_file and util.parse_vars_file(VARS_FILE)) or {}

local function read_vars_raw(name)
  local val = (type(VARS) == "table") and VARS[name] or nil
...
--[[ gtex62-tech-hud: unused daily cache helper (forecast path not used)
...
--[[ gtex62-tech-hud: unused conky_theme helper (no lua_parse theme usage)
...
--[[ gtex62-tech-hud: unused arc/icon geometry helpers (no arc draw path)
...
--[[ gtex62-tech-hud: unused planets opts (arc draw path)
...
-- Time formatting (24h)
----------------------------
--[[ gtex62-tech-hud: unused sunrise/sunset formatter
...
--[=[ gtex62-tech-hud: unused horizon arc geometry
-- =====================================================================
...
function conky_owm(key)
  ...
  --[[ gtex62-tech-hud: unused description key
  ...
  --[[ gtex62-tech-hud: unused wind key
  if key == "wind" then return read_field(".wind.speed") or "" end
  ]]

  -- Sunrise / sunset
  --[[ gtex62-tech-hud: unused sunrise/sunset keys
  ...
  -- Drawing entrypoints
  --[=[ gtex62-tech-hud: unused arc draw entrypoints
  ...
  -- Debug
  --[=[ gtex62-tech-hud: unused arc debug
...
--[=[ gtex62-tech-hud: unused horizon arc draw path
-- =====================================================================
...
function conky_owm_draw_horizon()
  ...
  do
    ...
    local function get_moon_times()
      local home = os.getenv("HOME") or ""
      local cmd  = SUITE_DIR .. "/scripts/moon_times.sh"
  ...
  -- Planet helpers (read radii and angles from ${CONKY_CACHE_DIR}/sky.vars)
...
--[=[ gtex62-tech-hud: unused read_forecast_5 helper (not referenced)
-- --------------------------------------------------
-- read_forecast_5()
...
local FC_PURGED = true
local DAY0_STATE_VARS = CACHE_DIR .. "/owm_day0_peak.vars"
local DAY0_KEY = nil
local DAY0_PEAK = nil
local DAY0_PEAK_REACHED = false
local function read_day0_state()
  local f = io.open(DAY0_STATE_VARS, "r")
...
local function write_day0_state(day_key, peak, reached)
  if not day_key or not peak then return end
  local f = io.open(DAY0_STATE_VARS, "w")
  if not f then return end
  f:write("DATE=", day_key, "\n")
  f:write("PEAK=", tostring(peak), "\n")
  f:write("REACHED=", reached and "1" or "0", "\n")
  f:close()
end
...
function conky_owm_draw_forecast_placeholder()
  ...
  local dow_pt = tonumber(tget(THEME, "weather.forecast.font_size"))

  local font_face = tget(THEME, "weather.forecast.font")
  if font_face == nil or font_face == "" or font_face == "auto" then
    font_face = (THEME.fonts and (THEME.fonts.value_c or THEME.fonts.value)) or "DejaVu Sans Mono"
  end
  local temps_font = tget(THEME, "weather.forecast.temps.font")
  if temps_font == nil or temps_font == "" or temps_font == "auto" then
    temps_font = (THEME.fonts and (THEME.fonts.value_c or THEME.fonts.value)) or font_face
  end
  local temps_weight = tget(THEME, "weather.forecast.temps.weight")
  local temps_weight_cairo = CAIRO_FONT_WEIGHT_NORMAL
  if temps_weight and tostring(temps_weight):lower() == "bold" then
    temps_weight_cairo = CAIRO_FONT_WEIGHT_BOLD
  end
  local dow_weight = tget(THEME, "weather.forecast.dow_weight")
  local dow_weight_cairo = CAIRO_FONT_WEIGHT_NORMAL
  if dow_weight and tostring(dow_weight):lower() == "bold" then
    dow_weight_cairo = CAIRO_FONT_WEIGHT_BOLD
  end

  -- text helper
  local function draw_centered_text(x, y, text, pt, color, weight, face)
    cairo_select_font_face(cr, face or font_face, CAIRO_FONT_SLANT_NORMAL, weight or CAIRO_FONT_WEIGHT_NORMAL)
  ...
  local current_temp = tonumber(read_field(".main.temp") or "")
  if current_temp then current_temp = math.floor(current_temp + 0.5) end
  local day_key = os.date("%Y-%m-%d")
  if day_key ~= DAY0_KEY then
    DAY0_KEY = day_key
    DAY0_PEAK = nil
    DAY0_PEAK_REACHED = false
    local vars = read_day0_state()
    if vars and vars.DATE == day_key then
      local peak = tonumber(vars.PEAK or "")
      if peak then DAY0_PEAK = peak end
      if vars.REACHED == "1" then DAY0_PEAK_REACHED = true end
    end
  end
  ...
  for i = 0, tiles - 1 do
    ...
    -- temps
    local hi_val = d and tonumber(d.hi) or nil
    local lo_val = d and tonumber(d.lo) or nil
    local hi_txt = hi_val and string.format("%.0f", hi_val) or "72°"
    if i == 0 and current_temp then
      local prev_peak = DAY0_PEAK
      local prev_reached = DAY0_PEAK_REACHED
      if DAY0_PEAK == nil then DAY0_PEAK = current_temp end
      if current_temp > DAY0_PEAK then DAY0_PEAK = current_temp end
      if hi_val and DAY0_PEAK and hi_val > DAY0_PEAK then
        DAY0_PEAK_REACHED = false
      end
      if hi_val and current_temp >= hi_val then
        DAY0_PEAK_REACHED = true
      elseif (not hi_val) and DAY0_PEAK and current_temp >= DAY0_PEAK then
        DAY0_PEAK_REACHED = true
      end
      if DAY0_PEAK_REACHED and DAY0_PEAK and current_temp < DAY0_PEAK then
        hi_txt = "-"
      elseif hi_val and current_temp > hi_val then
        hi_txt = string.format("%.0f", current_temp)
      elseif hi_val then
        hi_txt = string.format("%.0f", hi_val)
      else
        hi_txt = string.format("%.0f", current_temp)
      end
      if DAY0_PEAK and (prev_peak ~= DAY0_PEAK or prev_reached ~= DAY0_PEAK_REACHED) then
        write_day0_state(day_key, DAY0_PEAK, DAY0_PEAK_REACHED)
      end
    end
...
--[=[ gtex62-tech-hud: unused sun label draw
-- =====================================================================
...
function conky_taf()
  ...
  local wrap_cont = wrap - cols - indent
  if wrap_cont and wrap_cont < 8 then wrap_cont = nil end

  local function wrap_line(line, width)
    if not width or width <= 0 then return { line } end
    local out = {}
    local s = line
    while #s > width do
      local split_pos = 0
  ...
  local function push_line(line)
    ...
    if maxl and maxl > 0 and #out_lines >= maxl then
      return true
    end
    return false
  end
  ...
  for i = 1, #lines do
    local prefix = (i == 1) and pad or (pad .. hang)
    local chunks = (i > 1) and wrap_line(lines[i], wrap_cont) or { lines[i] }
    for _, chunk in ipairs(chunks) do
      if push_line(prefix .. chunk) then
        if #out_lines > 0 then
          out_lines[#out_lines] = out_lines[#out_lines] .. "…"
        end
