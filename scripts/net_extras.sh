#!/usr/bin/env bash
# ${CONKY_SUITE_DIR}/scripts/net_extras.sh
...
wan_status() {
  ...
  # Non-VPN default route: if we *have* a default route but no WAN cache yet,
  # show "...scanning" until the cache is populated.
  if [ -n "$gw_iface" ] && [ ! -s "$CACHE" ]; then
    echo "...scanning"
    return
  fi
...
wan_ip_label() {
  local state_file="$CONKY_CACHE_DIR/vpn_state"
  local raw state
