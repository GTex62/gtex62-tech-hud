...
read LAT LON <<<"$("$CONKY_SUITE_DIR/scripts/station_latlon.sh" "$STATION" 2>/dev/null || echo "")"
[ -n "${LAT:-}" ] && [ -n "${LON:-}" ] || exit 0

# Ensure we have a fresh-ish cache
"$CONKY_SUITE_DIR/scripts/airsig_fetch.sh" >/dev/null 2>&1 || true
CACHE="$CONKY_CACHE_DIR/airsigmets.json"
[ -s "$CACHE" ] || exit 0
